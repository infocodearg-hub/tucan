...
import { dayOfWeek, isWithin } from '../lib/date.js';
...
export function selectClientStats(state, clienteId) {
  ...
  const base = cliente.historicoPrevio ?? { partidos: 0, cancelaciones: 0, gastado: 0 };
  ...
  return {
    partidos: base.partidos + partidosVivos,
    cancelaciones: base.cancelaciones + cancelacionesVivas,
    gastado: base.gastado + gastadoVivo,
  };
}
...
export function selectDayKpis(state, fecha) {
  ...
  const totalSlots = (state.config?.operacion?.slots?.length ?? 0) * selectCanchasActivas(state).length;
