      ...
      <div className="modal-content" style={{ maxWidth: 480 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18, paddingBottom: 14, borderBottom: '1px solid var(--border-dim)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{
              ...
              color: isFullyPaid ? '#00B0FF' : '#FFB300'
            }}>
              <Calendar size={18} />
            </div>
            <div>
              <h3 className="font-heading" style={{ fontSize: '1.1rem', fontWeight: 900, color: '#fff' }}>
                {booking.clientName}
              </h3>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, fontSize: '0.82rem' }}>
            ...
            <span style={{ fontWeight: 700, color: '#fff' }}>${booking.totalPrice.toLocaleString('es-AR')}</span>
          </div>

          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, fontSize: '0.82rem' }}>
            <span style={{ color: 'var(--text-muted)' }}>Abonado / Seña:</span>
            <span style={{ fontWeight: 700, color: '#00E676' }}>${booking.depositPaid.toLocaleString('es-AR')}</span>
          </div>

          {booking.cantinaExtras?.length > 0 && (
            <div style={{ borderTop: '1px dashed var(--border-dim)', paddingTop: 6, marginTop: 6 }}>
              <p style={{ fontSize: '0.72rem', color: '#C8FF00', fontWeight: 800, marginBottom: 4 }}>
              ...
              {booking.cantinaExtras.map((item, idx) => (
                <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.78rem', color: 'var(--text-muted)' }}>
                  ...
                  <span style={{ color: '#fff' }}>${(item.price * item.qty).toLocaleString('es-AR')}</span>
                </div>
              ))}
            </div>
          )}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: 8, marginTop: 8, borderTop: '1px solid var(--border-dim)', fontSize: '0.95rem' }}>
            <span style={{ fontWeight: 800, color: '#fff' }}>Resta Cobrar en Puerta:</span>
            <span className="font-heading" style={{ fontWeight: 900, color: balance > 0 ? '#FFB300' : '#00E676', fontSize: '1.2rem' }}>
        ...
        {showAddCantina && (
          <div style={{ padding: 12, borderRadius: 12, background: 'var(--bg-surface)', border: '1px solid var(--border-dim)', marginBottom: 16 }}>
            <p style={{ fontSize: '0.74rem', fontWeight: 800, color: '#C8FF00', marginBottom: 8 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          {!isFullyPaid && (
            ...
            >
              <CheckCircle2 size={16} style={{ color: '#040A06' }} /> Saldar Restante (${balance.toLocaleString('es-AR')})
            </button>
          )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            ...
            >
              <ShoppingBag size={14} color="#C8FF00" /> + Cantina
            </button>

            <button
              onClick={handleWhatsApp}
              className="btn-secondary"
              style={{ justifyContent: 'center', padding: '9px', fontSize: '0.78rem', gap: 5, color: '#00E676', borderColor: 'rgba(0,230,118,0.3)' }}
            >
          ...
          <button
            ...
            style={{
              ...
              color: '#FF4F4F', padding: '9px', borderRadius: 10, fontSize: '0.78rem', fontWeight: 800,
